...
def jsonl_to_dataframe_with_intent(jsonl_text: str) -> pd.DataFrame:
    ...
    if not pure_lines:
        return pd.DataFrame(columns=["发言时间","玩家ID","玩家消息","一级分类","话题簇"])
    ...
    # 缺列补空
    for c in ["发言时间","玩家ID","玩家消息","一级分类","话题簇"]:
    ...
    # 只保留目标列顺序
    return df[["发言时间","玩家ID","玩家消息","一级分类","话题簇"]]
...
def _normalize_records(records: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    # 只保留需要列、并按既定顺序
    return df[["一级分类", "发言时间", "玩家ID", "玩家消息", "话题簇"]]
...
def append_json_to_excel_by_cat_and_tag(records: List[Dict[str, Any]], excel_path: str):
    ...
    for ca, grp in df.groupby("一级分类", sort=False):
        ...
        # 仅保留写入列顺序
        body_df = grp[["发言时间", "玩家ID", "玩家消息", "话题簇"]].copy()
...
def _pre_unmerge_and_fill_topic(ws):
    ...
    for rng in list(ws.merged_cells.ranges):
        if rng.min_col == 4 and rng.max_col == 4:
            to_process.append(rng)
    for rng in to_process:
        top_val = ws.cell(row=rng.min_row, column=4).value
        for r in range(rng.min_row, rng.max_row + 1):
            ws.cell(row=r, column=4).value = top_val
...
def _remove_all_drawings(ws):
    ...
    if hasattr(ws, "_charts"): ws._charts = []
    # 2) 删除 drawing 关系（避免残留）
    try:
    ...
    for i in range(1, n):
        ...
        if same_topic:
            if pd.notna(cur_ts) and pd.notna(last_ts):
                ...
                if cont: last_ts = cur_ts
            elif pd.isna(cur_ts):
                # NaT 行：skip=并入不更新；break=断段
...
# ========== 单表：排序→重写→切段→合并→标记🖼️→移除图片 ==========
# ========== 单表：排序→重写→切段→合并→标记🖼️→移除图片 ==========
def _sort_merge_flag(
    ws,
    gap_minutes: int = 15,
    nat_policy: str = "skip",          # "skip"：NaT 并入但不更新基准；"break"：NaT 直接断段
    fill_nat_in_topic: bool = True,    # 是否在同簇内对少量 NaT 做前后填补
    dump_bad_ts: bool = False          # 是否导出解析失败样本
):
    ...
    if ws.max_row >= 2:
        for r in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1, max_col=4, values_only=True):
            if r is None or all(x in (None, "") for x in r):
                continue
            rows.append(r)
    df = pd.DataFrame(rows, columns=["发言时间","玩家ID","玩家消息","话题簇"])
    if df.empty:
        if ws.max_row > 1:
            ws.delete_rows(2, ws.max_row - 1)
    ...
    # ======= 新版：规范 + 强韧解析 + 话题簇优先排序 =======
    # 1) 话题簇文本规范化（全角/零宽/多空格清理）
    df["话题簇"] = df["话题簇"].apply(_norm_topic)

    # 2) 强韧解析时间：先宽松，再按固定格式补一轮
    ...
    # 可选：导出坏样本，方便定位（只导当前 sheet）
    if dump_bad_ts:
        bad2 = df["_ts"].isna()
        if bad2.any():
            df.loc[bad2, ["发言时间","玩家ID","玩家消息","话题簇"]].to_excel(
                f"bad_ts_{ws.title}.xlsx", index=False
            )

    # 3) 仅当“该话题簇内至少有一个可解析时间”时才做填补，避免整簇 NaT 被错误填充
    if fill_nat_in_topic:
        def _safe_fill(s: pd.Series) -> pd.Series:
            if s.notna().any():
                return s.ffill().bfill()
            return s  # 整簇全 NaT：保持 NaT，不做填补
        df["_ts"] = df.groupby("话题簇")["_ts"].transform(_safe_fill)

    # 4) 严格排序：话题簇 → 时间 → 原串兜底（保证簇内时间升序）
    #    空话题簇会排在最前，如果你希望空话题簇排最后，可先打个标记后排序。
    df["_blank"] = (df["话题簇"] == "") | df["话题簇"].isna()
    df = df.sort_values(
        by=["_blank", "话题簇", "_ts", "发言时间"],
        ascending=[True, True, True, True],
        kind="mergesort"
    ).drop(columns=["_blank"])
    # ======= 以上是关键改动 =======
    ...
    for row in df.drop(columns=["_ts"]).itertuples(index=False, name=None):
        ws.append(row)
    for rr in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1, max_col=4):
        for cell in rr:
            cell.style = style

    # 切段（包含单行段；_iter_topic_runs 会再次保险地解析时间并严格 15min 判定）
    runs = _iter_topic_runs(df[["发言时间","话题簇"]].copy(), gap_minutes, nat_policy)

    # 先清旧合并，再按 runs 合并（单行段不会合并，但仍会标记）
    _clear_topic_merges(ws)
    for (s, e, _t) in runs:
        r1, r2 = s + 2, e + 2  # df 行号→Excel 行号
        if r2 > r1:
            ws.merge_cells(start_row=r1, start_column=4, end_row=r2, end_column=4)

    # 标记：逐行给纯图片行加 🖼️；逐段（单/多行）若含纯图片→给段首 D{r1} 加 🖼️
    for (s, e, _t) in runs:
        r1, r2 = s + 2, e + 2
        has_img_only = False
        for r in range(r1, r2 + 1):
            msg = ws.cell(row=r, column=3).value  # C列 = 玩家消息
            if _is_pure_image_msg(msg):
                has_img_only = True
                ws.cell(row=r, column=3).value = _strip_trailing_flag(msg) + " 🖼️"
        if has_img_only:
            tval = ws.cell(row=r1, column=4).value  # D列段首
            ws.cell(row=r1, column=4).value = _strip_trailing_flag(tval) + " 🖼️"

    # 最后彻底移除表里所有 shape，避免 Excel 仍显示小图片
    _remove_all_drawings(ws)
