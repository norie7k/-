...
# 提取“纯簇名”（去掉日期时间轴）
# 例如：
# "特殊重启契约书获取与区别讨论（2025-11-20 22:30:26-22:31:28）"
# -> "特殊重启契约书获取与区别讨论"
title_pattern = re.compile(
    r"^(?P<title>.+?)（\d{4}-\d{2}-\d{2}\s+[\d:]+-[\d:]+(?:、[\d:]+-[\d:]+)*）$"
)
...
def extract_clusters_from_output(output_text: str) -> List[Dict]:
    """
    从模型输出中提取“话题簇名称 + 相关描述”，用于更新白名单。
    - 自动去掉话题簇里的日期+时间轴，只保留主题名
    - 描述字段统一写入 '相关描述'
    """
    if not output_text:
        return []
    ...
    def _push(obj: Dict):
        raw_cluster = (obj.get("话题簇") or "").strip()
        # 有的模型可能直接输出 '话题簇名称'
        raw_cluster = raw_cluster or (obj.get("话题簇名称") or "")
        cluster_name = normalize_cluster_name(raw_cluster)

        # 兼容两种描述字段
        desc = (obj.get("核心对象/机制")
                or obj.get("相关描述")
                or "").strip()
...
def update_and_save_whitelist(path: Path,
                              current: List[Dict],
                              new_items: List[Dict]) -> List[Dict]:
    """
    current: 已有白名单（list[dict]）
    new_items: extract_clusters_from_output 的结果
               [{"话题簇名称": "...", "相关描述": "..."}]
    """
    existing_names = {item.get("话题簇名称") for item in current}
    added = [
        item for item in new_items
        if item.get("话题簇名称") and item["话题簇名称"] not in existing_names
    ]
...
##############转为jonsl输出#############
def attach_cluster_to_messages(
    聚合话题簇列表: List[Dict],
    原始发言: List[str],
    ensure_unique: bool = True
) -> List[Dict]:
    """
    根据聚合/校正后的话题簇（带 日期 + 时间轴），
    去原始发言中匹配对应时间段内的玩家发言，
    输出逐句带【话题簇 + 描述】的记录列表。

    ensure_unique: 若为 True，则同一条消息（通过 _idx 或 (日期+时间+玩家ID)）只允许命中一个话题簇。
    """
    # 1）解析原始 JSONL -> dict
    parsed_msgs: List[Dict] = [json.loads(line.strip()) for line in 原始发言 if line.strip()]
    ...
    for cluster in 聚合话题簇列表:
        ...
        topic_name = cluster.get("话题簇") or cluster.get("聚合话题簇")
     # 视你的字段命名而定

        if not (date and time_axis and topic_name):
            continue
        ...
        for msg in matched:
            # 只给玩家发言打话题簇
            if msg.get("发言人身份") and msg.get("发言人身份") != "玩家":
                continue

            # 生成一个唯一键，防止重复打簇
            # 优先用 _idx，如果没有再退化到 (日期, 时间, 玩家ID, 玩家消息)
            key = msg.get("_idx")
            if key is None:
                key = (msg.get("发言日期"), msg.get("发言时间"), msg.get("玩家ID") or msg.get("发言人ID"), msg.get("玩家消息"))

            if ensure_unique and key in assigned_keys:
                # 已经在别的簇里被用过了，就跳过（按“一个发言只能归一个簇”的规则）
                continue
            ...
            result = {
                "发言日期": msg.get("发言日期"),
                "发言时间": msg.get("发言时间"),
                "玩家ID": msg.get("玩家ID") or msg.get("发言人ID"),
                "玩家消息": msg.get("玩家消息"),
                "一级分类": msg.get("一级分类"),
                "话题簇": topic_name,
            }
...
cluster_regex = re.compile(
    r"(?P<title>.+?)（(?P<date>\d{4}-\d{2}-\d{2}) (?P<time_axis>[\d:]+-[\d:]+(?:、[\d:]+-[\d:]+)*)）"
)

def parse_clusters_from_output(output_text: str) -> List[Dict]:
    """
    把模型#3 校正输出 JSONL 文本解析为结构化簇：
    {"话题簇":"怪诞虫开局体验（2025-12-03 14:00:00-14:00:15）","核心对象/机制":"..."}
    ->
    {
      "话题簇": "怪诞虫开局体验",
      "日期": "2025-12-03",
      "时间轴": "14:00:00-14:00:15",
      "核心对象/机制": "..."
    }
    """
    clusters = []
    for line in output_text.splitlines():
        ...
        raw_topic = obj.get("话题簇", "").strip()
        desc = obj.get("核心对象/机制", "").strip()

        m = cluster_regex.match(raw_topic)
        if not m:
            continue

        clusters.append({
            "话题簇": m.group("title").strip(),
            "日期": m.group("date"),
            "时间轴": m.group("time_axis"),
            "核心对象/机制": desc,
        })
    return clusters
...
def time_in_ranges(date_str: str, time_str: str, time_axis_str: str) -> bool:
    ts = datetime.strptime(f"{date_str} {time_str}", "%Y-%m-%d %H:%M:%S")
    segments = time_axis_str.split("、")
    for seg in segments:
        start_dt, end_dt = parse_time_range(date_str, seg)
...
# ---- 当前批次：簇 + 原文 → 逐句结果 ----
def attach_cluster_to_batch(
    cluster_list: List[Dict],
    batch_raw_lines: List[str],
    cls_map: Dict[Tuple, int],
) -> List[Dict]:
    raw_msgs = [json.loads(line.strip()) for line in batch_raw_lines if line.strip()]
    results = []
    assigned_keys = set()
    ...
    for msg in raw_msgs:
        ...
        date = msg.get("发言日期")
        t = msg.get("发言时间")
        pid = msg.get("玩家ID") or msg.get("发言人ID")
        text = msg.get("玩家消息")
        ...
        for clu in cluster_list:
            if clu["日期"] != date:
                continue
            if time_in_ranges(date, t, clu["时间轴"]):
                topic_name = clu["话题簇"]
                break
        ...
        classifytag = cls_map.get(key)  # ⭐ 从分类 map 里取标签
        ...
        results.append({
            "发言日期": date,      # 可保留，以后如果要用还在
            "发言时间": dt_str,    # ✅ Excel 用这个列，包含日期+时间
            "玩家ID": pid,
            "玩家消息": text,
            "一级分类": classifytag,
            "话题簇": topic_name,
...
#########分类####################
def build_class_map_from_text(output_class_text: str) -> Dict[Tuple, int]:
    """
    把模型#2 的输出（这一批的分类 JSONL 文本）做成：
    key = (发言日期, 发言时间, 玩家ID, 玩家消息)
    val = 分类标签 / 一级分类

    自动处理：
    - 去掉所有 Markdown 代码块的反引号（``` / ```json 之类）
    - 忽略非 JSON 行（说明文字、空行等）
    """
    cls_map: Dict[Tuple, int] = {}

    if not output_class_text:
        return cls_map

    # 1️⃣ 先把所有反引号干掉，彻底清掉 ``` / ```json 这类东西
    #    （JSON 里本来也用不到 ` 字符）
    s = output_class_text.replace("`", "").strip()

    # 2️⃣ 提取“看起来是 JSON 对象的一行”
    pure_lines: List[str] = []
    ...
    for line in pure_lines:
        ...
        key = (
            obj.get("发言日期"),
            obj.get("发言时间"),
            obj.get("玩家ID") or obj.get("发言人ID"),
            obj.get("玩家消息"),
        )

        # 兼容字段名：有的叫“分类标签”，有的叫“一级分类”
        val = obj.get("分类标签", obj.get("一级分类"))
        cls_map[key] = val
